...
func _process(delta: float) -> void:
	...
	stamina.text = str(Globals.stamina)
	if Globals.health < 0 or Globals.stamina < 0:
		run_success_fail.text = "ran out of health/stamina. you loose"
		run_success_fail.visible = true
		await get_tree().create_timer(3.0).timeout
		run_success_fail.visible = false
		get_tree().quit()
	if Globals.enemy_health < 0 and attacks_used.visible == false:
